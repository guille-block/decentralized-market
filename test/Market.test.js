...
contract('Market', ([deployer, seller, buyer]) => {
    ...
    describe('Buying a product', async () => {
        ...
        it('Correct balances', async () => {
           ...
           assert.equal(price, web3.utils.toWei('1', 'Ether') , 'Price not equal')
           const gas = new web3.utils.BN(bought.receipt.gasUsed)
           oldBalance = oldBalance.add(price)
           assert.equal(oldBalance.toString(), newBalance.toString(), 'Balance is not equal')
        })    
    })
})